...
begin
	...
	begin
		...
		w_stop <= '1'; wait for k_clk_period;
		      assert w_floor = x"4" report "bad stop at floor 4" severity failure;
        
        -- go all the way down DOWN (how many clock cycles should that take?)
        w_up_down <= '0';
        w_stop    <= '0';
        wait for k_clk_period * 3;
            assert w_floor = x"1" report "bad down from floor 4 to 1" severity failure;
            
        -- elevator stopping on bottom floor while going down
        w_stop <= '1';
        wait for k_clk_period;
            assert w_floor = x"1" report "bad keep at floor 1 from floor 1" severity failure;
